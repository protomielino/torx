...
Layer *ANN_AddLayer(ANN * ann, int n_inputs, int n_outputs, real * x)
{
	...
	ListAppend_hook(ann->c, (void *) l, (void *) &ANN_FreeLayer);
	return l;
}
...
void ANN_FreeLayer_low_level(void *l)
{
...
ANN* LoadANN_fname(char* filename)
{
	FILE* f = fopen (filename, "rb");
	if (f) {
		ANN* ann = LoadANN_fdesc (f);
		fclose (f);
		return ann;
	}
	return NULL;
...
int SaveANN_fname(ANN* ann, char* filename)
{
	FILE* f = fopen (filename, "wb");
	if (f) {
		int r = SaveANN_fdesc (ann, f);
		fclose (f);
		return r;
	}
	return -1;
...
int SaveANN_fdesc(ANN* ann, FILE* f)
{
	if (f==NULL) {
		return -1;
	}
